...
export default function AiMognadmatningProjektPage() {
  return (
...
function ProjektListContent() {
  ...
  useEffect(() => {
    fetch("/api/assessments?type=ai-mognad")
      .then((r) => r.json())
      .then((data) => {
          ...
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                {projects.length > 0
                  ? `${projects.length} projekt för AI-Mognadsmätning`
                  : "Inga projekt skapade ännu"}
              </p>
            </div>
            <Link href="/ai-mognadmatning">
      ...
      <div className="px-8 py-6">
        {loading ? (
          ...
          <EmptyState
            icon="brain"
            title="Inga projekt"
            description="Skapa ditt första projekt för att börja mäta AI-mognad."
            action={
              <Link href="/ai-mognadmatning">
                <Button className="gap-2">
                  <Icon name="plus" size={16} />
          ...
          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
            {projects.map((p) => (
              <Link key={p.id} href={`/ai-mognadmatning/projekt/${p.id}`} className="group block">
