...
export default function AiMognadmatningResultatPage({
  params,
}: {
  ...
  return (
    <FeatureGate featureKey="ai-mognadmatning.results">
...
function ResultatContent({ sessionId }: { sessionId: string }) {
  ...
  if (!data) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] px-8 text-center">
        ...
        <Link href="/ai-mognadmatning/projekt" className="text-sm text-primary hover:underline">
  ...
  const levelColors: Record<number, string> = {
    ...
    4: "text-violet-600 bg-violet-100 dark:bg-violet-900/30 dark:text-violet-400",
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="border-b border-border/60 bg-card/60">
        <div className="px-8 py-8">
          <div className="flex items-center gap-2 text-sm text-muted-foreground mb-3">
            ...
            <Link
              href={`/ai-mognadmatning/projekt/${session.project.id}`}
      ...
      <div className="px-8 py-8 max-w-4xl">
              ...
              return (
                <div key={dim}>
                  ...
                  <div className="h-3 rounded-full bg-muted overflow-hidden">
                    <div
                      className="h-full rounded-full bg-violet-600 dark:bg-violet-400 transition-all duration-500 ease-out"
        ...
        <div className="rounded-2xl border border-border/60 bg-card p-6 shadow-sm mb-6">
          <h2 className="text-sm font-semibold text-foreground mb-4">AI-mognadsnivåer</h2>
          <div className="space-y-3">
            {config.maturityLevels.map((ml) => (
              <div
                ...
                className={`flex items-start gap-3 rounded-xl border p-3 transition-colors ${
                  ml.level === result.level
                    ? "border-violet-400/40 bg-violet-50 dark:bg-violet-900/20"
              ...
              >
                ...
                <div>
                  <p className={`text-sm font-medium ${ml.level === result.level ? "text-violet-700 dark:text-violet-300" : "text-foreground"}`}>
                    {ml.name}
                  </p>
        ...
        <div className="rounded-2xl border border-border/60 bg-card p-6 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-sm font-semibold text-foreground flex items-center gap-2">
              <Icon name="brain" size={16} className="text-violet-600 dark:text-violet-400" />
              AI-insikter
            </h2>
            {!result.aiInsights && (
              <Button
