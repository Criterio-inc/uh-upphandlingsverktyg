  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/assessments", {
        ...
        body: JSON.stringify({
          assessmentTypeSlug: "digital-mognad",
          name: form.name.trim(),
      ...
      router.push(`/mognadmatning/projekt/${data.project.id}`);
    } catch {
      setLoading(false);
    }
  }
        ...
        <div className="px-8 py-8">
          <div className="flex items-center gap-3 mb-2">
            <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-emerald-100 dark:bg-emerald-900/30">
              <Icon name="bar-chart-3" size={20} className="text-emerald-600 dark:text-emerald-400" />
            </div>
            <div>
      ...
      <div className="px-8 py-8 max-w-3xl">
        ...
        <div className="grid gap-4 sm:grid-cols-3 mb-8">
          <div className="rounded-2xl border border-border/60 bg-card p-4 shadow-sm">
            <div className="flex items-center gap-2.5">
              <div className="flex h-8 w-8 items-center justify-center rounded-xl bg-emerald-100 dark:bg-emerald-900/30">
                <Icon name="file-text" size={15} className="text-emerald-600 dark:text-emerald-400" />
              </div>
              <div>
          ...
          <div className="rounded-2xl border border-border/60 bg-card p-4 shadow-sm">
            <div className="flex items-center gap-2.5">
              <div className="flex h-8 w-8 items-center justify-center rounded-xl bg-emerald-100 dark:bg-emerald-900/30">
                <Icon name="target" size={15} className="text-emerald-600 dark:text-emerald-400" />
              </div>
              <div>
          ...
          <div className="rounded-2xl border border-border/60 bg-card p-4 shadow-sm">
            <div className="flex items-center gap-2.5">
              <div className="flex h-8 w-8 items-center justify-center rounded-xl bg-emerald-100 dark:bg-emerald-900/30">
                <Icon name="clock" size={15} className="text-emerald-600 dark:text-emerald-400" />
              </div>
              <div>
                <p className="text-[11px] text-muted-foreground">Tidsåtgång</p>
                <p className="text-lg font-semibold text-foreground leading-tight">~10 min</p>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {config.dimensions.map((dim) => (
              <div key={dim} className="flex items-start gap-3 rounded-xl border border-border/40 bg-card/50 p-3">
                <div className="flex h-6 w-6 shrink-0 items-center justify-center rounded-lg bg-emerald-100 dark:bg-emerald-900/30 mt-0.5">
                  <Icon name="check" size={12} className="text-emerald-600 dark:text-emerald-400" />
                </div>
                <div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="t.ex. Mognadsmätning Q1 2025"
