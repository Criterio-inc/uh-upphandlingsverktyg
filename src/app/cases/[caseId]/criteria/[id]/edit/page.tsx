...
export default async function EditCriterionPage({
  ...
  const item = await prisma.criterion.findUnique({ where: { id } });
  if (!item) notFound();

  const meta = getEntityMeta("criterion");
  const basePath = `/cases/${caseId}/criteria`;
  ...
  return (
    <div>
      ...
      <div className="max-w-2xl p-6">
        <EntityForm
          meta={meta}
          apiUrl={`/api/cases/${caseId}/criteria/${id}`}
